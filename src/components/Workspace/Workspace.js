...
function Workspace({ stepConfig, reference, editable = false, icons }) {
    ...
    <div className="f-screen-appbar flex flex-col gap-3 xl:gap-7 items-center mx-auto lg:max-w-7xl lg:items-stretch lg:flex-row">
      {stepConfig.config.map((el, index) => {
        return (
          <div
            key={index}
            className={`flex flex-col gap-1 lg:gap-5 w-full lg:px-2 xl:px-0 overflow-hidden  ${sizes[el.size]}`}
          >
            <Panel
              tools={el.tools}
